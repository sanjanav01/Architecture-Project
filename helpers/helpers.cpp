...
void replaceInfrequentColors(ColorChannels& channels, const std::map<std::tuple<int, int, int>, int>& color_freq,
    int frequency_threshold) {

  for (size_t i = 0; i < channels.R.size(); ++i) {
    auto color = std::make_tuple(channels.R[i], channels.G[i], channels.B[i]);
    auto itt = color_freq.find(color);

    if (itt != color_freq.end() && itt->second < frequency_threshold) {
      std::cout << "Infrequent color found: (" << channels.R[i] << ", " << channels.G[i] << ", " << channels.B[i] << ")\n";

      auto [newR, newG, newB] = findClosestColor(color, color_freq, frequency_threshold);
      std::cout << "Replacing with closest frequent color: (" << newR << ", " << newG << ", " << newB << ")\n";
...
std::tuple<int, int, int> findClosestColor(
  ...
  std::tuple<int, int, int> closest_color = color;
