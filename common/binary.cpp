...
namespace {
// Helper function to read the PPM header
void readHeader(std::ifstream& file, Image& image) {
...
// Helper function to read 8-bit pixel data
void read8BitPixels(std::ifstream& file, Image& image, bool scale) {
    for (auto& pixel : image.pixels) {
        std::array<uint8_t, RGB_CHANNELS> rgb{};
        // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
        file.read(reinterpret_cast<char*>(rgb.data()), RGB_CHANNELS);
        if (file.gcount() != RGB_CHANNELS) {
            throw std::runtime_error("Error reading pixel data for 8-bit color");
        }
        if (scale) {
            pixel = {.r = static_cast<uint8_t>(rgb[0] * MaxByteValue / image.max_color_value),
                     .g = static_cast<uint8_t>(rgb[1] * MaxByteValue / image.max_color_value),
                     .b = static_cast<uint8_t>(rgb[2] * MaxByteValue / image.max_color_value)};
        } else {
...
// Helper function to read 16-bit pixel data
void read16BitPixels(std::ifstream& file, Image& image) {
    for (auto& pixel : image.pixels) {
        std::array<uint8_t, RGB_CHANNELS_16BIT> rgb_bytes{};
        // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
        file.read(reinterpret_cast<char*>(rgb_bytes.data()), RGB_CHANNELS_16BIT);
        if (file.gcount() != RGB_CHANNELS_16BIT) {
            throw std::runtime_error("Error reading pixel data for 16-bit color");
        }
        const auto red = static_cast<uint16_t>((rgb_bytes[0] << 8) | rgb_bytes[1]);
        const auto green = static_cast<uint16_t>((rgb_bytes[2] << 8) | rgb_bytes[3]);
        const auto blue = static_cast<uint16_t>((rgb_bytes[4] << 8) | rgb_bytes[5]);
        pixel = {.r = static_cast<uint8_t>(red * MaxByteValue / image.max_color_value),
                 .g = static_cast<uint8_t>(green * MaxByteValue / image.max_color_value),
                 .b = static_cast<uint8_t>(blue * MaxByteValue / image.max_color_value)};
    }
}
...
Image read_ppm(const std::string& file_path) {
    ...
    if (image.max_color_value < MaxByteValue) {
        read8BitPixels(file, image, true); // Read and scale
    } else if (image.max_color_value == MaxByteValue) {
        read8BitPixels(file, image, false); // Read without scaling
    } else {
        read16BitPixels(file, image); // Read 16-bit data
    }
...
void write_ppm(const std::string& file_path, const Image& image) {
  ...
  for (const auto& pixel : image.pixels) {
    if (use_1_byte_per_channel) {
      ...
      out_file.put(static_cast<char>(pixel.r & LOW_BYTE_MASK))
              .put(static_cast<char>((pixel.r >> HIGH_BYTE_SHIFT) & LOW_BYTE_MASK))
              .put(static_cast<char>(pixel.g & LOW_BYTE_MASK))
              .put(static_cast<char>((pixel.g >> HIGH_BYTE_SHIFT) & LOW_BYTE_MASK))
              .put(static_cast<char>(pixel.b & LOW_BYTE_MASK))
              .put(static_cast<char>((pixel.b >> HIGH_BYTE_SHIFT) & LOW_BYTE_MASK));
    }
  }
  if (!out_file) {
...
CompressedImage read_cppm(const std::string& file_path) {
  ...
  //NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
    ...
    size_t index_byte_length = 0;
    if (color_table_size <= LE_MinMaxByteValue) {index_byte_length = 1;
    } else if (color_table_size <= LE_MaxByteValue) {index_byte_length = 2;
    } else {index_byte_length = 4;}
  while (file) {
        uint32_t index = 0;
        if (index_byte_length == 1) {
            uint8_t idx = 0;
